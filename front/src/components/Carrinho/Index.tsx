...
import { useCarrinho } from "../../hooks/useCardProduto";
...
export default function Carrinho({
	...
	const {
		...
		taxaEntrega,
		total,
	} = useCarrinho(); // useCardProduto hook to manage cart items	
			...
			<div className="fixed inset-0 z-50 flex justify-end bg-black/70">
				<div className="h-full w-full max-w-md bg-white p-6 relative overflow-auto scrollbar-thin scrollbar-thumb-transparent scrollbar-track-transparent">
					...
					<div className="flex items-center justify-between">
						...
						<Button color="plain" className="p-2 w-28">
							Ver Cardapio
						</Button>
					</div>
						...
						{dados.map((item: any) => (
							<div key={item.id} className="p-4">
								<div className="flex items-center justify-between">
									...
									<p className="">R$ {item.subtotal.toFixed(2)}</p>
								</div>
								...
								<div className="flex items-center mt-2 gap-2">
									...
									<Button color="plain" className="p-1 w-20" onClick={() => removerItem(item.id)}>
										Remover
									</Button>
								</div>
							</div>
