...
export function useCardProdutos(produto: IProduto, restaurante: IRestaurante) {
  ...
  async function adicionarAoCarrinho() {
    const enderecoId = localStorage.getItem("enderecoPadraoId");
    ...
    if (enderecoId) {
      ...
      showWarning("Por favor, selecione um endereço antes de adicionar ao carrinho.");
    }
  }
  ...
  const incrementar = () => {
    if (quantidade < produto.qtd_estoque) {
      ...
      console.warn("Estoque máximo atingido.");
      showWarning("Você atingiu o limite disponível em estoque.");
    }
  };
